...
class Breaker extends Events.EventEmitter {
  constructor(impl, options) {
    ...
    if (options && options.maxFailures && options.maxFailureThreshold) {
      throw new Error('Circuit breaker cannot be configured to use both an absolute value and a threshold.');
    }
  ...
  _run(/*args...n, callback*/) {
    ...
    try {
      this._thresholdCounter.numRequests += 1;
  ...
  _onFailure() {
    if (this.settings.maxFailureThreshold) {
      this._thresholdCounter.numFailures += 1;

      // TODO: figure out a better mechanism for calculating threshold
      const currThreshold = this._thresholdCounter.numRequests === 1 ? 0 : (this._thresholdCounter.numFailures / this._thresholdCounter.numRequests) * 100;

      if (currThreshold >= this.settings.maxFailureThreshold) {
        this.open();
      }
    } else {
      this._numFailures += 1;
